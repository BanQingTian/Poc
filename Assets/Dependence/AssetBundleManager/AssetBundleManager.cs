...
namespace AssetBundles
{
    ...
    public partial class AssetBundleManager : MonoBehaviour
    {
        ...
        public delegate AssetBundleLoadOperation OverrideAssetBundleDownloadOperationDelegate(string root, string assetBundleName);

        public static event OverrideAssetBundleDownloadOperationDelegate overrideAssetBundleDownloadOperation;
        ...
        {
            return Initialize(Utility.AssetBundlesDefaultRootName);
        }
        ...
        static public AssetBundleLoadManifestOperation Initialize(string manifestAssetBundleName)
        {
            ...
            LoadAssetBundle(Utility.AssetBundlesDefaultRootName, manifestAssetBundleName, true);
        ...
        static public void LoadAssetBundle(string root, string assetBundleName)
        {
            LoadAssetBundle(root, assetBundleName, false);
        }

        // Starts the download of the asset bundle identified by the given name, and asset bundles
        // that this asset bundle depends on.
        static protected void LoadAssetBundle(string root, string assetBundleName, bool isLoadingAssetBundleManifest)
        {
            ...
            if (SimulateAssetBundleInEditor)
                return;
#endif
            if (root.Equals(Utility.AssetBundlesDefaultRootName))
            {
            ...
            bool isAlreadyProcessed = LoadAssetBundleInternal(root, assetBundleName, isLoadingAssetBundleManifest);

            // Load dependencies.
            if (!isAlreadyProcessed && !isLoadingAssetBundleManifest)
                LoadDependencies(root, assetBundleName);
        }
        ...
        protected static AssetBundleLoadOperation GetAssetBundleDownloadOperation(string root, string assetBundleName)
        {
            if (overrideAssetBundleDownloadOperation != null)
            {
                foreach (OverrideAssetBundleDownloadOperationDelegate method in overrideAssetBundleDownloadOperation.GetInvocationList())
                {
                    AssetBundleLoadOperation res = method(root, assetBundleName);
                    if (res != null)
                        return res;
                }
            }
        ...
        static protected bool LoadAssetBundleInternal(string root, string assetBundleName, bool isLoadingAssetBundleManifest)
        {
            // Already loaded.
            LoadedAssetBundle bundle = null;
            m_LoadedAssetBundles.TryGetValue(assetBundleName, out bundle);
            ...
            {
                AssetBundleLoadOperation operation = GetAssetBundleDownloadOperation(root, assetBundleName);
                if (operation == null)
                {
                    WWW download = null;
                    ...
                    operation = new AssetBundleDownloadFromWebOperation(assetBundleName, download);
                }
        ...
        static protected void LoadDependencies(string root, string assetBundleName)
        {
            if (!root.Equals(Utility.AssetBundlesDefaultRootName))
            {
                return;
            ...
            for (int i = 0; i < dependencies.Length; i++)
                LoadAssetBundleInternal(root, dependencies[i], false);
        }
        ...
        /// </summary>
        static public AssetBundleLoadAssetOperation LoadAssetAsync(string root, string assetBundleName, string assetName, System.Type type)
        {
            ...
            if (SimulateAssetBundleInEditor)
            {
                // Remap variant name in simulation mode.
                string[] assetPaths = { };
                for (int i = 0; i < m_ActiveVariants.Length; ++i)
                {
                    string assetBundleNameWithVariant = string.Format("{0}.{1}", assetBundleName, m_ActiveVariants[i]);
                    assetPaths = AssetDatabase.GetAssetPathsFromAssetBundleAndAssetName(assetBundleNameWithVariant, assetName);
        ...
        static public AssetBundleLoadAssetOperation<T> LoadAssetAsync<T>(string root, string assetBundleName, string assetName) where T : UnityEngine.Object
        {
            Log(LogType.Info, "Loading " + assetName + " from " + assetBundleName + " bundle");
        ...
        /// </summary>
        static public AssetBundleLoadOperation LoadLevelAsync(string root, string assetBundleName, string levelName, bool isAdditive)
        {
